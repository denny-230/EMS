...
class Employee:
  def __init__(self,root):
    ...
    img_logo=Image.open('emp123\logo1.png')
    ...
    img1=Image.open('colleg_images\employeee1.png')
    ...
    img2=Image.open('colleg_images\employee3.jpg')
    ...
    img3=Image.open('colleg_images\image123.png')
    img3=img3.resize((535,150),Image.ANTIALIAS)
   ...
   # upper frame
    ...
    # Inserted Image
    img6=Image.open('colleg_images\einstein.jpg')
   ...
   #   search frame
    ...
    combo_txt_search=ttk.Combobox(search_frame,textvariable=self.var_search_com,font=('arial', 12,'bold'),width=17,state='readonly')
    combo_txt_search['value']=('Select','Phone','Aadhar Card')
    ...
    btn_search=Button(search_frame,width=14,command=self.search_data,font=('arial',11,'bold'),text='Search',bg='blue',fg='white')
       ...
       messagebox.showerror('Error','All fields are mandatory')
        ...
        try:
             ...
             if update>0:
                conn=mysql.connector.connect(host='localhost',username='root',password='Test@123',database='new_data')
                my_cursor=conn.cursor()
                my_cursor.execute('update person set Department=%s,Name=%s,Designition=%s,Email=%s,Address=%s,Married=%s,Dob=%s,Doj=%s,Idtype=%s,Phone=%s,Country=%s,Salary=%s,Gender=%s where Idproof=%s',(
             ...
             self.fetch_data()
             conn.close()
             messagebox.showinfo('Success','Employ has been updated successfully',parent=root)
        except Exception as es:
             messagebox.showerror('Error',f'Due To:{str(es)}',parent=root)
  ...
  def delete_data(self): 
     if self.var_idproof.get()=="":
       messagebox.showerror('Error','All fields are mandatory')
         ...
         try:
             Delete=messagebox.askyesno('Delete','Are you sure to delete this employee data',parent=root)
             if Delete>0:
                conn=mysql.connector.connect(host='localhost',username='root',password='Test@123',database='new_data')
                my_cursor=conn.cursor()
                sql='delete from person where Idproof=%s'
             ...
             conn.commit()
             conn.close()
             messagebox.showinfo('Success','Employ has been deleted successfully',parent=root)
             self.fetch_data()
         except Exception as es:
             messagebox.showerror('Error',f'Due To:{str(es)}',parent=root)
  ...
  def reset_data(self): 
       ...
       self.var_married.set("Maritial Status"),
       self.var_dob.set(""),
       self.var_doj.set(""),
  ...
  def search_data(self):
     if self.var_search.get()=="" or self.var_search_com.get()=="":
          messagebox.showerror('Please select one option')
     else:
       try:  
             conn=mysql.connector.connect(host='localhost',username='root',password='Test@123',database='new_data')
             my_cursor=conn.cursor()
             my_cursor.execute('select * from person where' +str(self.var_search_com.get())+ "LIKE'%"+str(self.var_search.get()+"%'"))
